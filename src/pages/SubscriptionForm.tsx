...
export default function SubscriptionForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = {
      ...
      billingMonth:
        state.billingCycle === "annually" ? state.billingMonth : undefined,
  ...
  const existingMethods = [
    ...new Set(subscriptions.map((s) => s.paymentMethod).filter(Boolean)),
  ];
  const paymentSuggestions = [
    ...new Set([...existingMethods, ...STATIC_PAYMENT_METHODS]),
  ];
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {state.billingCycle === "annually" && (
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    v && set({ billingMonth: parseInt(v, 10) })
                  }
                >
            ...
            <div className="space-y-2">
              ...
              <Select
                value={
                  state.addingNewMethod ? "__add_new__" : state.paymentMethod
                }
                onValueChange={(v) => {
                  if (!v) return;
                  if (v === "__add_new__")
                    set({ addingNewMethod: true, paymentMethod: "" });
